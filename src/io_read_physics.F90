      !*************** Changed by Adolfo ************
      SUBROUTINE io_read_physics_config(this,phys,d_random, stat_info)
!      SUBROUTINE io_read_physics_config(this,phys,stat_info)
        ...
        !********** Added by Adolfo **********
        TYPE(Random), INTENT(IN)     :: d_random
        !***********************************
...
!             cc_magnet_mu      = 1.0
          ...
          !**** Added by Adolfo *********
!!$          ELSE IF ( carg == 'COLL_BOUNCE_BACK'  .AND. &
!!$               num_species > 1 .AND. &
!!$               num_colloid > 0 ) THEN
!!$             
!!$             !-----------------------------------------------
!!$             ! increase of pressure to ensure bounce back
!!$             !-----------------------------------------------
!!$             
!!$             READ(cvalue,*,IOSTAT=ios, ERR=200) coll_bounce_back
         !***************************
          ...
          ELSE IF(carg == 'CC_REPUL_F0' .AND. &
             ...
             !*********** Added by Adolfo *****
          ELSE IF(carg == 'CC_QUAD_K' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0) THEN
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_quad_k
             !********************************

             !*********** Added by Adolfo *****
          ELSE IF(carg == 'CC_QUAD_R0' .AND. &
               num_species > 1 .AND. &
               num_colloid > 0) THEN
             
             READ(cvalue,*,IOSTAT=ios,ERR=200) cc_quad_r0
             !********************************
        ...
        IF ( stat_info == 0 ) THEN
           ...
           IF ( num_species > 1 .AND. num_colloid > 0 ) THEN
              ...
              CALL colloid_new(colloids,&
                   ...
                   coll_integrate_type,&
                   coll_integrate_RK,&
                   coll_integrate_AB, d_random, stat_info_sub)
!!$              CALL colloid_new(colloids,&
!!$                   num_dim,num_colloid,&
!!$                   coll_integrate_type,&
!!$                   coll_integrate_RK,&
...
!!$              CALL colloid_set_bounce_back(colloids, &
!!$                   coll_bounce_back,stat_info_sub)
		   !*************************************
...
!                   coll_implicit_pair_sweep_tolerance,stat_info_sub)
		   !*************************************
              ...
              !******* Added by Adolfo *******
              CALL colloid_set_cc_quad_k(colloids, &
                   cc_quad_k,stat_info_sub)
              CALL colloid_set_cc_quad_r0(colloids, &
                   cc_quad_r0,stat_info_sub)
              !*******************************
