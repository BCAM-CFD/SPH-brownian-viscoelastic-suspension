...
!-------------------------------------------------
      ...
      !********** Added by Adolfo *************
      INTEGER FUNCTION colloid_get_bounce_back(this, stat_info)
        !----------------------------------------------------
        ! Return the increase on the pressure to ensure a bounce back
        !----------------------------------------------------
        ...
        stat_info = 0
        colloid_get_bounce_back = this%bounce_back
        
        RETURN
        
      END FUNCTION colloid_get_bounce_back
!*****************************
...
!*********** Added by Adolfo *************
      SUBROUTINE colloid_get_max_phys(this,d_max_phys,stat_info)
        !----------------------------------------------------
        ! Return the positions of colloids.
        !----------------------------------------------------
        ...
        d_max_phys(:) = &
             this%max_phys(1:this%num_dim)
        
        RETURN
        
      END SUBROUTINE colloid_get_max_phys
      !*************************************************

!*********** Added by Adolfo *************
      SUBROUTINE colloid_get_min_phys(this,d_min_phys,stat_info)
        !----------------------------------------------------
        ! Return the positions of colloids.
        !----------------------------------------------------
        ...
        d_min_phys(:) = &
             this%min_phys(1:this%num_dim)
        
        RETURN
        
      END SUBROUTINE colloid_get_min_phys
      !*************************************************
...
9999    CONTINUE
	...
	!*********************
	 ...
	 ! and here there are only 4. **
        !** I think there is not a problem adn it should be 4 ***
	 !--- Modified by Adolfo ---
!!!         ALLOCATE(d_vector(4,this%num_colloid))
!	 IF (dim == 2) THEN
            ALLOCATE(d_vector(4,this%num_colloid))
!!$	 ELSE !-- dim = 3 --
!!$            ALLOCATE(d_vector(9,this%num_colloid))
!!$	 ENDIF
