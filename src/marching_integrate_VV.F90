...
!                            colloid_get_implicit_pair_sweep_error_tang(colloids,stat_info_sub)
	...
	!-- Added by Adolfo --
	! I do not know why, but this allows the code to be stable.
	! Probably some non-initialized variable.
!	aux = .FALSE.
!	IF (aux) THEN
!	       	WRITE(*,*) 'A'
!		ENDIF		
...
9999	CONTINUE
	...
	!----------------------------------------------------
        ...
        IF (ASSOCIATED(max_phys)) THEN
           DEALLOCATE(max_phys)
        END IF
        IF (ALLOCATED(total_force_bottom)) THEN
           DEALLOCATE(total_force_bottom)
        END IF
        IF (ALLOCATED(total_force_top)) THEN
