	...
	REAL(MK)                                :: beta
        ...
        IF ( num_sweep < 1 ) THEN
           PRINT *, __FILE__,__LINE__,&
                "number of sweeps is wrong!", num_sweep
           stat_info = -1
           GOTO 9999
        END IF
...
!        F0_repul = this%cc_repul_F0
	!*****************************
        ...
        !-- Added by Adolfo -- **** This can be changed by any required viscosity ***
        eta_total = this%eta
...
!!$        !-- normal lubrication with thermal noise (brownian)
!!$	A3D = -6.0_MK * mcf_pi * eta_total * &
!!$	      ( ai*aj/aa )**2.0_MK *dt_sweep
!!$        A3D_kt = ( ai*aj/aa ) * Sqrt(12.0_MK * mcf_pi * kt * this%eta / dim)
...
!!$	    (15.0_MK * (1.0_MK + beta)**3.0_MK)*dt_sweep
	 !-- Brownian normal and tangential lubrication --
	A3D = -6.0_MK * mcf_pi * eta_total * &
	      ( ai*aj/aa )**2.0_MK 
	beta = aj / ai
	B3D = -6.0_MK * mcf_pi * eta_total * ai * &
	    (1.0_MK + 7.0_MK * beta + beta * beta) / &
	    (5.0_MK * (1.0_MK + beta)**3.0_MK)
	C3D = -6.0_MK * mcf_pi * eta_total * ai * &
	    (4.0_MK * beta * (2.0_MK + beta + &
	    2.0_MK * beta * beta)) / &
	    (15.0_MK * (1.0_MK + beta)**3.0_MK)
        A3D_kt = sqrt(-4.0_MK * kt * C3D)
	!-- End of added by Adolfo --
         

        !----- Added by Adolfo --------
        ALLOCATE(dW_overline(dim,dim))
        ALLOCATE(dPij_vec(dim))
        !------------------------------
...
!!$			   ENDIF 
                          ...
                          !-- This is normal lubrication brownian case --
!!$                          trace = 0.0_MK
!!$                          DO K = 1, dim
...
!!$                                (1.0_MK / this%m(i) + &
!!$				1.0_MK / this%m(j))
!!$                           aux = 1.0_MK / (Aij - 1.0_MK)
!!$                           !dPij in absolute value
!!$                           dPij = A3D_kt * SQRT(( 1.0_MK/h - &
!!$                                1.0_MK/hn_l )) * trace
!!$                           aux_kt = dPij * (1.0_MK / this%m(i) + &
!!$				1.0_MK / this%m(j)) * aux / num_sweep

                          !-- Brownian case with normal and tangential lubrication --
                          aux  = (LOG(ai/h) - LOG(ai/hn_l))
                          aux2 = (1.0_MK / this%m(i) + 1.0_MK / this%m(j))
                          fij  = (A3D * ( 1.0_MK/h - 1.0_MK/hn_l ) + &
                               B3D * aux)
                          trace=0.0_MK
                          DO K = 1, dim
                             trace = trace + dWij(i,j,k,k)
                          ENDDO
                          IF (h < htang_l) THEN
                             !--note that this aux is different then the previous one --
                             aux  = (LOG(ai/h) - LOG(ai/htang_l))
                             gij  = C3D * aux 
                             DO K = 1, dim
                                DO L = 1, dim
                          ...
                          B_kt = sqrt(2.0_MK * dim * kt * (-fij + 2.0_MK*gij*(1.0_MK - 1.0_MK/dim)))
                          !dPij 3D case
                          DO K = 1, dim
                             dPij_vec(K) = A_kt * (dW_overline(K,1) * eij(1) + &
                                                dW_overline(K,2) * eij(2) + &
                                                dW_overline(K,3) * eij(3)) + &
                                                B_kt/dim * trace * eij(K)
                          ENDDO
                          Cij = fij * dt_sweep * aux2 
                          Dij = gij * dt_sweep * aux2 
