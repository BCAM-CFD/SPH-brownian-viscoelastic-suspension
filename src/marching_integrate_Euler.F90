...
!*****************************************
        ...
        REAL(MK), DIMENSION(:), POINTER :: total_force_top, total_force_bottom                
        !*************************
        ...
        NULLIFY(min_phys)
        NULLIFY(max_phys)
        NULLIFY(total_force_bottom)
        NULLIFY(total_force_top)                
        !***********************        
        NULLIFY(bcdef)
        NULLIFY(tboundary)
        wall_drag_p(:,:) = 0.0_MK
              ...
              DO i = 1, coll_sub_time_step                 
                 ...
                 CASE (-2)
                    ...
                    CALL colloid_compute_interaction_implicit_velocity_pair(&
                         colloids, rank, comm, MPI_PREC, step, dt_sub_time_step,&
                         coll_drag,coll_torque, &
                         wall_drag_c(1:num_dim,1:num_dim*2),kt, stat_info_sub)
!!$                    CALL colloid_compute_interaction_implicit_velocity_pair(&
...
9999    CONTINUE
	...
	!----------------------------------------------------
        ...
        IF (ASSOCIATED(max_phys)) THEN
           DEALLOCATE(max_phys)
        END IF
        IF (ASSOCIATED(total_force_bottom)) THEN
           DEALLOCATE(total_force_bottom)
        END IF
        IF (ASSOCIATED(total_force_top)) THEN
           DEALLOCATE(total_force_top)
        END IF                
        !***********************
