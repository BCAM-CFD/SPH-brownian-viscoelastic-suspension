      ...
      SUBROUTINE colloid_compute_interaction_implicit_velocity_pair(this,&
           rank, comm,MPI_PREC,step,dt,drag,torque,FB,kt,stat_info)
!!$      SUBROUTINE colloid_compute_interaction_implicit_velocity_pair(this,&
!!$           comm,MPI_PREC,step,dt,drag,torque,FB,stat_info)
	...
	!********** Added by Adolfo ***********
        ...
        REAL(MK), DIMENSION(:,:), ALLOCATABLE :: torque_tang_lub
        INTEGER :: l
        REAL(MK) :: C3D
        REAL(MK) :: eta_total
        REAL(MK) :: aux2, aux
        ...
        REAL(MK) :: A_kt
        REAL(MK) :: dW_overline_dot_eij_dot_eij
        REAL(MK), DIMENSION(:), ALLOCATABLE :: dW_overline_dot_eij
        REAL(MK), DIMENSION(3) :: vij
        REAL(MK), DIMENSION(3) :: force
        REAL(MK), DIMENSION(3) :: dtorque
        REAL(MK) :: eij_vij
        REAL(MK) :: beta
	!**************************************
	...
	!****************************************
        ...
        !**** Added by Adolfo *****
        ALLOCATE(dWij(this%num_colloid, this%num_colloid, dim, dim))
        ! The values of dWij are calculated 
        ai = this%radius(1,1)
        aj = ai
        aa = ai+aj
        ...
        !    subroutine. ---
        IF (rank == 0) THEN
           DO i = 1, this%num_colloid - 1
              DO j = i + 1, this%num_colloid
                 ...
                 h  = r - aa
                 IF ( h < this%cc_lub_cut_off ) THEN
                    DO k = 1, dim
                       DO l = 1, dim
                          dWij(i,j,k,l) = random_random(this%random,stat_info_sub)
                       END DO
                    END DO
                    dWij(i,j,:,:) = dWij(i,j,:,:) * SQRT(dt) !Random normal distribution with 
        ...
        IF ( this%cc_lub_type > mcf_cc_lub_type_no ) THEN
           ...
           CALL colloid_compute_interaction_implicit_velocity_pair_sweep(&
                this, dt, num_sweep, dWij, kt, stat_info_sub)
!!$           CALL colloid_compute_interaction_implicit_velocity_pair_sweep(&
!!$                this, dt, num_sweep, stat_info_sub)
           ...
           IF ( sweep_adaptive ) THEN
              ...
              DO  WHILE ( num_sweep1 >= 1 .AND. &
                 ...
                 !************ Changed by Adolfo ********
                 CALL colloid_compute_interaction_implicit_velocity_pair_sweep(&
                      this, dt, num_sweep1, dWij, kt, stat_info_sub)
!!$                    CALL colloid_compute_interaction_implicit_velocity_pair_sweep(&
!!$                         this, dt, num_sweep1, stat_info_sub)
              ...
              ELSE
                 ...
                 !************* Changed by Adolfo ************
                 CALL colloid_compute_interaction_implicit_velocity_pair_sweep(&
                      this, dt, num_sweep2, dWij, kt, stat_info_sub)
!!$                 CALL colloid_compute_interaction_implicit_velocity_pair_sweep(&
...
!**** Commented because it is not clear is correct ***
!!$        !***** Added by Adolfo ****
!!$        ! Very simple way to introduce a change in the angular velocity due to the tangential
!!$        ! lubrication force between colloids. 
!!$        !---------------------
!!$
!!$        !--The total torque due to tangential lubrication forces is calculated --
!!$        ALLOCATE(torque_tang_lub(this%num_colloid, dim))
!!$        ALLOCATE(dW_overline(dim,dim))
!!$        ALLOCATE(dW_overline_dot_eij(dim))
!!$        !-- ** Esto se puede cambiar por el valor de viscosidad de lubricacion que se necesite **
!!$        eta_total = this%eta 
!!$
!!$        beta = aj / ai
...
!!$	    2.0_MK * beta * beta)) / &
!!$	    (15.0_MK * (1.0_MK + beta)**3.0_MK)
!!$        htang_l  = this%cc_lub_cut_off/4.0_MK
!!$        torque_tang_lub(:,:) = 0.0_MK
!!$        DO i = 1, this%num_colloid - 1
!!$           DO j = i + 1, this%num_colloid
!!$              CALL colloid_nearest_image(this,&
!!$                   this%x(1:dim,i),j, &
...
!!$              r  = SQRT(DOT_PRODUCT(rij(1:dim), rij(1:dim)))
!!$              h  = r - aa
!!$              IF ( h < htang_l ) THEN !-- tangential lub cutoff --
!!$                 !-- Brownian case with normal and tangential lubrication --
!!$                 aux2 = (1.0_MK / this%m(i) + 1.0_MK / this%m(j))
!!$                 trace = 0.0_MK
!!$                 DO K = 1, dim
!!$                    trace = trace + dWij(i,j,k,k)
!!$                 ENDDO
!!$                 aux  = (LOG(ai/h) - LOG(ai/htang_l))
!!$                 gij  = C3D * aux 
!!$                 DO K = 1, dim
!!$                    DO L = 1, dim
!!$                       dW_overline(k,l) = 0.5_MK * (dWij(i,j,k,l) + dWij(i,j,l,k))
!!$                    ENDDO
!!$                    dW_overline(k,k) = dW_overline(k,k) - trace/real(dim, kind = MK)
!!$                 ENDDO
!!$                 A_kt = sqrt(-4.0_MK * kt * gij)
!!$                 dW_overline_dot_eij_dot_eij = 0.0_MK
!!$                 DO K = 1, dim
!!$                    dW_overline_dot_eij(K) = dW_overline(K,1) * eij(K) + &
!!$                         dW_overline(K,2) * eij(2) + dW_overline(K,3) * eij(3)
!!$                    dW_overline_dot_eij_dot_eij = dW_overline_dot_eij_dot_eij + &
!!$                         dW_overline_dot_eij(K) * eij(K)        
!!$                 ENDDO
!!$                 vij(1:dim) = this%v(1:dim,i,1) - this%v(1:dim,j,1)
!!$                 eij_vij = eij(1) * vij(1) + &
!!$                      eij(2) * vij(2) + &
!!$                      eij(3) * vij(3)
!!$
!!$                 DO K = 1, 3
!!$                    force(K) = gij * (vij(K) - eij_vij * eij(K)) + &
!!$                         1.0_MK/dt * A_kt * (dW_overline_dot_eij(K) -  &
!!$                         dW_overline_dot_eij_dot_eij * eij(K))
!!$                 ENDDO
!!$
!!$                 !** Supposing the force applied on the surface of the sphere ***
!!$                 dtorque(1) = -aj*(eij(2)*force(3) - eij(3)*force(2))
!!$                 dtorque(2) = -aj*(eij(3)*force(1) - eij(1)*force(3)) 
!!$                 dtorque(3) = -aj*(eij(1)*force(2) - eij(2)*force(1)) 
!!$                 torque_tang_lub(I,:) = torque_tang_lub(I,:) + dtorque(:)
!!$                 torque_tang_lub(J,:) = torque_tang_lub(J,:) - dtorque(:)
!!$              ENDIF
!!$           END DO
!!$        END DO
!!$        
!!$        !-- Differently than for repulsion I do not use two substeps in here, just one --
!!$        DO I = 1, this%num_colloid
!!$           this%omega(1:dim,I,1) = this%omega(1:dim,I,1) + &
!!$                torque_tang_lub(I,1:dim)/this%mmi(1:dim,I) * dt
!!$        ENDDO
        ...
        IF (ALLOCATED(dWij)) THEN
           DEALLOCATE(dWij)
        ENDIF
        IF (ALLOCATED(torque_tang_lub)) THEN
           DEALLOCATE(torque_tang_lub)
        ENDIF
        IF (ALLOCATED(dW_overline)) THEN
           DEALLOCATE(dW_overline)
        ENDIF
        !************************************
